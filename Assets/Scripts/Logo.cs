...
public class Logo : MonoBehaviour
{
	...
	private bool CheckPermissions()
	{
		for (int i = 0; i < Permissions.Length; i++)
		{
			if (!AndroidNativeFunctions.CheckPermission("android.permission." + Permissions[i]))
			{
				...
				AndroidNativeFunctions.ShowAlert("For the game to work need permission: " + Permissions[i], "Block Strike", "OK", string.Empty, string.Empty, delegate
				{
					Application.Quit();
				});
				return true;
	...
	private bool CheckGame()
	{
		...
		{
			Detected("Xposed_detected");
			return true;
		}
		percentLabel.text = "35%";
		...
		for (int i = 0; i < installedApps.Length; i++)
		{
			...
			{
				Detected("No Find App: " + installedApps[i]);
				return true;
			}
		}
		...
		for (int j = 0; j < array.Length; j++)
		{
			...
			{
				Detected("Virtual Detected", array[j]);
				return true;
			}
	...
	private bool CheckGame2()
	{
		...
		if (array.Length < 20)
		{
			Detected("Error Device App");
			return true;
		}
		if (AndroidNativeFunctions.GetFilesDir().Contains("999"))
		{
			Detected("Error Device App 2");
			return true;
		}
		...
		try
		{
			...
			for (int i = 0; i < processesByName.Length; i++)
			{
				...
				if (text2 != "su")
				{
					...
					{
						StartCoroutine(Check(sourceDir + "!/res/raw/chunk8", text2));
					}
				}
			}
		}
		...
		{
			if (appList.Contains(array[l]))
			{
				Detected(AndroidNativeFunctions.GetAppName(array[l]) + " detected", array[l]);
				return true;
			}
			...
			if ((FileExists(array[l], "lib0.so") && FileExists(array[l], "lib1.so")) || (FileExists(array[l], "lib2.so") && FileExists(array[l], "lib3.so")) || FileExists(array[l], "lib4.so") || FileExists(array[l], "lib7.so") || FileExists(array[l], "lib8.so"))
			{
				Detected("Game Guardian detected");
				return true;
			}
			...
			{
				if (FileExists(array[l], "libgg_time_32.so") || FileExists(array[l], "libgg_tibe.so"))
				{
					Detected("Game Guardian detected");
					return true;
				}
				if (FileExists(array[l], "libgg_time.so"))
				{
					Detected("Game Guardian detected", array[l]);
					return true;
				}
				if (FileExists(array[l], "lib-bulldog-daemon-exe.so"))
				{
					Detected("Game Guardian detected", array[l]);
					return true;
				}
				if (File.Exists("data/data/" + array[l] + "/files/version.gg"))
				{
					Detected("Game Guardian detected");
					return true;
				}
				if (FileExists(array[l], "libgltools.so") || FileExists(array[l], "libjustuseless.so"))
				{
					Detected(AndroidNativeFunctions.GetAppName(array[l]) + " detected", array[l]);
					return true;
				}
				if (FileExists(array[l], "liblbs.so") && FileExists(array[l], "libmarsxlog.so"))
				{
					Detected(AndroidNativeFunctions.GetAppName(array[l]) + " detected", array[l]);
					return true;
				}
				if (FileExists(array[l], "libexec.so"))
				{
					if (!FileExists(array[l], "libunrar.so") && !File.Exists("data/data/" + array[l] + "/proxy.sh") && !File.Exists("data/data/" + array[l] + "/iptables"))
					{
						Detected(AndroidNativeFunctions.GetAppName(array[l]) + " detected", array[l]);
						return true;
					}
				...
				{
					if (FileExists(array[l], "libencode.so"))
					{
						Detected(AndroidNativeFunctions.GetAppName(array[l]) + " detected", array[l]);
						return true;
					}
					if (FileExists(array[l], "libgamekiller.so"))
					{
						Detected(AndroidNativeFunctions.GetAppName(array[l]) + " detected", array[l]);
						return true;
					}
					if (FileExists(array[l], "libNative.so"))
					{
					...
					{
						if (FileExists(array[l], "libcecore.so"))
						{
							Detected(AndroidNativeFunctions.GetAppName(array[l]) + " detected", array[l]);
							return true;
						}
						if (FileExists(array[l], "libipadjust.so"))
						{
							Detected(AndroidNativeFunctions.GetAppName(array[l]) + " detected", array[l]);
							return true;
						}
						if (FileExists(array[l], "libMacro.so"))
						{
							Detected(AndroidNativeFunctions.GetAppName(array[l]) + " detected", array[l]);
							return true;
						}
						...
						if (FileExists(array[l], "libmhx.so") || FileExists(array[l], "libbig-stuff.so"))
						{
							Detected(AndroidNativeFunctions.GetAppName(array[l]) + " detected", array[l]);
							return true;
						}
						if (FileExists(array[l], "libdummysprite.so"))
						{
							Detected(AndroidNativeFunctions.GetAppName(array[l]) + " detected", array[l]);
							return true;
						}
