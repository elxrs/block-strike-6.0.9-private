...
public class UIPanel : UIRect
{
	...
	public Vector3 drawCallOffset
	{
		get
		{
...
#endif
			{
				...
				float num = ((!(root != null)) ? 1f : root.pixelSizeAdjustment);
				float num2 = num / windowSize.y / mCam.orthographicSize;
#if UNITY_5_5_OR_NEWER
				bool flag = false, flag2 = false;
#else
				bool flag = mHalfPixelOffset, flag2 = mHalfPixelOffset;
#endif
	...
	public Vector4 finalClipRegion
	{
		get
		{
			...
			Vector3 vector = anchorCamera.WorldToScreenPoint(cachedTransform.position);
	...
	public override Vector3[] worldCorners
	{
		get
		{
			...
			else
			{
				...
				{
					return mCam.GetWorldCorners(cameraRayDistance);
				}
				Vector2 viewSize = GetViewSize();
	...
	public override float CalculateFinalAlpha(int frameID)
	{
...
#endif		
		{
			...
			finalAlpha = ((!(parent != null)) ? mAlpha : (uIRect.CalculateFinalAlpha(frameID) * mAlpha));
		}
		return finalAlpha;
	}
	...
	{
		Transform transform = cachedTransform.parent;
...
#endif
		{
			UICamera uICamera = ((!(anchorCamera != null)) ? null : mCam.GetComponent<UICamera>());
	...
	private void UpdateTransformMatrix()
	{
		...
		if (mHasMoved || mMatrixFrame != num)
		{
			mMatrixFrame = num + 120;
			worldToLocal = cachedTransform.worldToLocalMatrix;
			Vector2 vector = GetViewSize() * 0.5f;
			float num2 = mClipOffset.x + mClipRange.x;
			float num3 = mClipOffset.y + mClipRange.y;
			mMin.x = num2 - vector.x;
			mMin.y = num3 - vector.y;
			mMax.x = num2 + vector.x;
			mMax.y = num3 + vector.y;
		}
		nProfiler.EndSample();
	}
    ...
    {
        if (!mEnabled || (widgetAreStatic && !widgetUpdateFrame && Application.isPlaying) || frameCount > fc || mUpdateFrame == fc)
        {
            return;
        }
#if UNITY_EDITOR
...
#endif
		mUpdateFrame = fc;
		frameCount = mUpdateFrame + frameCountSkip;
	...
	{
		if (widgetAreStatic && !widgetUpdateFrame && Application.isPlaying)
		{
			return;
		}
		nProfiler.BeginSample("UIPanel.UpdateSelf");
		widgetUpdateFrame = false;
	...
	private void UpdateDrawCalls(int sortOrder)
	{
		...
		if (flag)
		{
			Transform transform2 = cachedTransform.parent;
		...
		while (j < count)
		{
			...
			if (uiwidget.panel == this && uiwidget.enabled)
			{
				...
				if (!uiwidget.widgetAreStatic || uiwidget.widgetUpdateFrame || !Application.isPlaying)
				{
					...
					if (uiwidget.UpdateTransform(num) || mResized || (mHasMoved && !alwaysOnScreen))
					{
	...
	public void Refresh()
	{
		...
		if (list.Count > 0)
		{
			list[0].nLateUpdate(Time.frameCount);
		}
	}
