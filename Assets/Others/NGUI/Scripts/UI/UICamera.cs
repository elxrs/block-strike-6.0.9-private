...
public class UICamera : MonoBehaviour
{
	...
	public static GetTouchDelegate GetTouch = delegate (int id, bool createIfMissing)
	{
		...
		int j = 0;
		for (int size2 = mTouchIDs.size; j < size2; j++)
		{
			if (mTouchIDs.buffer[j] == id)
			{
	...
	{
		int i = 0;
		for (int size = mTouchIDs.size; i < size; i++)
		{
			if (mTouchIDs.buffer[i] == id)
			{
		...
		get
		{
			...
			if (mLastOverCheck != frameCount)
			{
				...
				for (int j = 0; j < 3; j++)
				{
					...
					if (IsPartOfUI((!(mouseOrTouch2.pressed != null)) ? mouseOrTouch2.current : mouseOrTouch2.pressed))
					{
						mLastOverResult = true;
						return mLastOverResult;
					}
	...
	public static GameObject hoveredObject
	{
		...
		set
		{
			...
			if ((bool)mHover)
			{
				if (flag)
				{
					UICamera uICamera2 = ((!(mHover != null)) ? list.buffer[0] : FindCameraForLayer(mHover.layer));
					if (uICamera2 != null)
					{
						current = uICamera2;
	...
	public static GameObject selectedObject
	{
		...
		set
		{
			...
			{
				UICamera uICamera2 = ((!(mSelected != null)) ? list.buffer[0] : FindCameraForLayer(mSelected.layer));
				if (uICamera2 != null)
				{
					current = uICamera2;
					currentCamera = uICamera2.cachedCamera;
	...
	public static bool Raycast(Vector3 inPos)
	{
		...
		for (int i = 0; i < list.size; i++)
		{
			...
			{
#if UNITY_4_7
				var mRayHits = Physics.RaycastAll(ray, enter, layerMask);
				var hitCount = mRayHits.Length;
#else
				if (mRayHits == null) mRayHits = new RaycastHit[50];
				var hitCount = Physics.RaycastNonAlloc(ray, mRayHits, enter, layerMask, QueryTriggerInteraction.Collide);
#endif
				if (hitCount > 1)
				{
					for (int j = 0; j < hitCount; j++)
					{
						var gameObject = mRayHits[j].collider.gameObject;
...
#if UNITY_4_7
				Collider2D[] mOverlap = Physics2D.OverlapPointAll(lastWorldPosition, layerMask);
				var hitCount = mOverlap.Length;
#else
				if (mOverlap == null) mOverlap = new Collider2D[50];
				var hitCount = Physics2D.OverlapPointNonAlloc(lastWorldPosition, mOverlap, layerMask);
#endif
				if (hitCount > 1)
				{
	...
	private void Start()
	{
		...
		if (Application.isPlaying)
		{
			if (fallThrough == null)
			{
				...
				fallThrough = ((!(uIRoot != null)) ? gameObject : uIRoot.gameObject);
			}
