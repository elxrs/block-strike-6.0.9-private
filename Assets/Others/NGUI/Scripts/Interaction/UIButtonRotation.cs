...
public class UIButtonRotation : MonoBehaviour
{
	...
	private void OnPress(bool isPressed)
	{
		if (enabled)
		{
			...
			TweenRotation.Begin(tweenTarget.gameObject, duration, isPressed ? (mRot * Quaternion.Euler(pressed)) : ((!UICamera.IsHighlighted(gameObject)) ? mRot : (mRot * Quaternion.Euler(hover)))).method = UITweener.Method.EaseInOut;
		}
	}
