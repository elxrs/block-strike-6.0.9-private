...
public class UIDragDropItem : MonoBehaviour
{
	...
	{
		if (!interactable || !enabled || mTouch != UICamera.currentTouch)
		{
			return;
		}
		if (restriction != 0)
	...
	public virtual UIDragDropItem StartDragging()
	{
		...
		if (!mDragging)
		{
			...
			{
				mPressed = false;
				GameObject gameObject = transform.parent.gameObject.AddChild(base.gameObject);
	...
	{
		if (interactable && mDragging && enabled && mTouch == UICamera.currentTouch)
		{
			if (mRoot != null)
			{
