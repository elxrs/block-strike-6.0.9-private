...
namespace CodeStage.AntiCheat.Detectors
{
	...
	public class WallHackDetector : ActDetectorBase
	{
		...
		public bool CheckRigidbody
		{
			...
			set
			{
				if (checkRigidbody == value || !Application.isPlaying || !enabled || !gameObject.activeSelf)
				{
					return;
				}
		...
		public bool CheckController
		{
			...
			set
			{
				if (checkController == value || !Application.isPlaying || !enabled || !gameObject.activeSelf)
				{
					return;
				}
		...
		public bool CheckWireframe
		{
			...
			set
			{
				if (checkWireframe == value || !Application.isPlaying || !enabled || !gameObject.activeSelf)
				{
					return;
				}
		...
		public bool CheckRaycast
		{
			...
			set
			{
				if (checkRaycast == value || !Application.isPlaying || !enabled || !gameObject.activeSelf)
				{
					return;
				}
				checkRaycast = value;
				...
				if (checkWireframe && wfCamera == null)
				{
					...
					else
					{
						...
						if (foregroundRenderer == null)
						{
							...
							foregroundRenderer.shadowCastingMode = UnityEngine.Rendering.ShadowCastingMode.Off;
						...
						if (backgroundRenderer == null)
						{
							...
							Destroy(gameObject2.GetComponent<MeshCollider>());
