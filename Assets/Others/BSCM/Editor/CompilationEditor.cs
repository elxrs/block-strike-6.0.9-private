...
public class CompilationEditor : EditorWindow
{
	...
	static void Init() 
	{
		GetWindow<CompilationEditor>(true, "Build Map", true);
	}
	...
	void CheckScene()
	{
		...
		GUIText[] guiText = FindObjectsOfType<GUIText>();
		for (int i = 0; i < guiText.Length; i++)
		{
			guiText[i].enabled = false;
		}
		
		GUITexture[] guiTexture = FindObjectsOfType<GUITexture>();
		for (int i = 0; i < guiTexture.Length; i++)
		{
			guiTexture[i].enabled = false;
		}
#endif
		...
		ParticleSystem[] particleSystem = FindObjectsOfType<ParticleSystem>();
		for (int i = 0; i < particleSystem.Length; i++)
		{
			particleSystem[i].gameObject.SetActive(false);
		}
		
		TrailRenderer[] trailRenderer = FindObjectsOfType<TrailRenderer>();
		for (int i = 0; i < trailRenderer.Length; i++)
		{
			trailRenderer[i].enabled = false;
		}
		
		LineRenderer[] lineRenderer = FindObjectsOfType<LineRenderer>();
		for (int i = 0; i < lineRenderer.Length; i++)
		{
			lineRenderer[i].enabled = false;
		}
		
		LensFlare[] lensFlare = FindObjectsOfType<LensFlare>();
		for (int i = 0; i < lensFlare.Length; i++)
		{
			lensFlare[i].enabled = false;
		}
		
		Projector[] projector = FindObjectsOfType<Projector>();
		for (int i = 0; i < projector.Length; i++)
		{
			projector[i].enabled = false;
		}
		
		SpriteRenderer[] spriteRenderer = FindObjectsOfType<SpriteRenderer>();
